...
def DataTransform(sample, config):
    ...
    weak_aug = scaling(sample, config.augmentation_jitter_scale_ratio)
    # weak_aug = permutation(sample, max_segments=config.augmentation.max_seg)
    strong_aug = jitter(permutation(sample, max_segments=config.augmentation_max_seg), config.augmentation_jitter_ratio)
...
def DataTransform_TD(sample, config):
    ...
    aug = jitter(sample, config.augmentation_jitter_ratio)
    return aug
...
def DataTransform_TD_bank(sample, config):
    ...
    aug_1 = jitter(sample, config.augmentation_jitter_ratio)
    aug_2 = scaling(sample, config.augmentation_jitter_scale_ratio)
    aug_3 = permutation(sample, max_segments=config.augmentation_max_seg)
...
def remove_frequency(x, pertub_ratio=0.0):
    ...
    mask = np.random.uniform(low=0.0, high=1.0, size=x.shape) > pertub_ratio # maskout_ratio are False
...
def add_frequency(x, pertub_ratio=0.0):
    ...
    mask = np.random.uniform(low=0.0, high=1.0, size=x.shape) > (1-pertub_ratio) # only pertub_ratio of all values are True
    # mask = mask.to(x.device)
    max_amplitude = x.max()
    # random_am = torch.rand(mask.shape)*(max_amplitude*0.1)
    random_am = np.random.rand(*mask.shape)*(max_amplitude*0.1)
...
def masking(x, keepratio=0.9, mask= 'binomial'):
    ...
    if mask == 'binomial':
        ...
        mask_id = generate_binomial_mask(x.size(0), x.size(1), x.size(2), p=keepratio)
